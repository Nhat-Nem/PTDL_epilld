...
class CBP(nn.Module):
     ...
     def forward(self, x):
         ...
         for img in range(batchSize // bsn):
             ...
             sketch1 = torch.fft.fft(
                torch.cat((sketch1, torch.zeros_like(sketch1)), dim=2),
                dim=1
             )

             sketch2 = batch_x.mm(self.sparseM[1].to(x.device)).unsqueeze(2)
             sketch2 = torch.fft.fft(
                torch.cat((sketch2, torch.zeros_like(sketch2)), dim=2),
                dim=1
             )

             fft_product = sketch1 * sketch2

             tmp_y = torch.fft.ifft(fft_product, dim=1).real
             tmp_y = tmp_y[:, :self.output_dim, 0]
